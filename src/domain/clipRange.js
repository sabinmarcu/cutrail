...
export const validateRange = (range, options = {}) => {
...
export const normalizeClipRanges = (ranges, options = {}) => {
  ...
  const validRanges = normalizedWithSourceIndex
  ...
  const errors = normalizedWithSourceIndex
    .filter((range) => !range.valid)
    .map((range) => ({
      id: range.id,
      sourceIndex: range.sourceIndex,
